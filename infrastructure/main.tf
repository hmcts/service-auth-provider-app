...
module "s2s-api" {
  ...
  app_settings = {
    ...
    MICROSERVICE_KEYS_CCD_ADMIN                  = "${data.vault_generic_secret.ccdAdmin.data["value"]}"
    MICROSERVICE_KEYS_CCD_DATA                   = "${data.vault_generic_secret.ccdData.data["value"]}"
    MICROSERVICE_KEYS_CCD_DEFINITION             = "${data.vault_generic_secret.ccdDefinition.data["value"]}"
    MICROSERVICE_KEYS_CCD_GW                     = "${data.vault_generic_secret.ccdGw.data["value"]}"
    MICROSERVICE_KEYS_CMC                        = "${data.vault_generic_secret.cmc.data["value"]}"
    MICROSERVICE_KEYS_CMC_LEGAL_FRONTEND         = "${data.vault_generic_secret.cmcLegalFrontend.data["value"]}"
    MICROSERVICE_KEYS_DIVORCE                    = "${data.vault_generic_secret.divorce.data["value"]}"
    MICROSERVICE_KEYS_DIVORCE_CCD_SUBMISSION     = "${data.vault_generic_secret.divorceCcdSubmission.data["value"]}"
    MICROSERVICE_KEYS_DIVORCE_FRONTEND           = "${data.vault_generic_secret.divorceFrontend.data["value"]}"
    MICROSERVICE_KEYS_DIVORCE_CCD_VALIDATION     = "${data.vault_generic_secret.divorceCcdValidation.data["value"]}"
    MICROSERVICE_KEYS_DIVORCE_DOCUMENT_UPLOAD    = "${data.vault_generic_secret.divorceDocumentUpload.data["value"]}"
    MICROSERVICE_KEYS_DIVORCE_DOCUMENT_GENERATOR = "${data.vault_generic_secret.divorceDocumentGenerator.data["value"]}"
    MICROSERVICE_KEYS_DRAFT_STORE_TESTS          = "${data.vault_generic_secret.draftStoreTests.data["value"]}"
    MICROSERVICE_KEYS_JOBSCHEDULER               = "${data.vault_generic_secret.jobscheduler.data["value"]}"
    MICROSERVICE_KEYS_SSCS                       = "${data.vault_generic_secret.sscs.data["value"]}"
    MICROSERVICE_KEYS_SSCS_TRIBUNALS_CASE        = "${data.vault_generic_secret.sscsTribunalsCase.data["value"]}"
    MICROSERVICE_KEYS_PROBATE_FRONTEND           = "${data.vault_generic_secret.probateFrontend.data["value"]}"
    MICROSERVICE_KEYS_PROBATE_BACKEND            = "${data.vault_generic_secret.probateBackend.data["value"]}"
    MICROSERVICE_KEYS_SEND_LETTER_CONSUMER       = "${data.vault_generic_secret.sendLetterConsumer.data["value"]}"
    MICROSERVICE_KEYS_SEND_LETTER_TESTS          = "${data.vault_generic_secret.sendLetterTests.data["value"]}"
    MICROSERVICE_KEYS_REFERENCE                  = "${data.vault_generic_secret.reference.data["value"]}"
    MICROSERVICE_KEYS_EM_GW                      = "${data.vault_generic_secret.emGw.data["value"]}"
    MICROSERVICE_KEYS_CMC_CLAIM_STORE            = "${data.vault_generic_secret.cmcClaimStore.data["value"]}"
    MICROSERVICE_KEYS_CCD_PS                     = "${data.vault_generic_secret.ccdPs.data["value"]}"
    MICROSERVICE_KEYS_FINREM                     = "${data.vault_generic_secret.finRem.data["value"]}"
